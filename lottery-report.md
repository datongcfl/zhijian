...
1. **bare_except** (5处) — 会吞掉 SystemExit/KeyboardInterrupt
   - rate_limit.py: 2处, settle.py: 3处
   - 修复: 改为 except Exception 或捕获具体异常

2. **nested_complexity** (20处) — 深嵌套+高复杂度
   - auth.py signup 嵌套7层
   - score_prediction_snapshot.py _prize_dlt 复杂度32
   - 修复: 提取子函数、使用 early return

### P1 (建议修复)

3. **god_function** (86处) — 函数太长
   - recommender.py: 5个上帝函数
   - fetch_and_update.py: 98行
   - 修复: 拆分为多个小函数

4. **phantom_import** (110处) — from src.xxx import ...
   - 运行时通过 sys.path 解决，静态分析认为是幻觉导入
   - 建议: 添加 src 到 packages 配置或使用相对导入

### P2 (可选优化)

5. function_clone_cluster (6处) — 重复代码
6. global_statement (8处) — 全局变量
7. pass_placeholder (3处) — 未实现的函数